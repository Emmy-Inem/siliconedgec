...
import { ROLE_LABEL, StaffRole } from "@/lib/admin-permissions";
...
const EDITABLE_ROLES: Exclude<StaffRole, "admin">[] = [
  "moderator", "instructor", "support", "finance", "content_editor",
];
...
export default function AdminPermissions() {
  ...
  const resetDefaults = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await (supabase as any).from("role_permissions").upsert(upserts, { onConflict: "role,route" });
      if (error) throw error;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Toggle which admin routes each staff role can access. Admin has full access.</p>
          </div>
        </div>
            ...
            <thead className="bg-muted/40 sticky top-0">
              <tr>
                ...
                {EDITABLE_ROLES.map((r) => (
                  <th key={r} className="text-center px-3 py-3 font-semibold whitespace-nowrap">{ROLE_LABEL[r]}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                        ...
                        return (
                          <td key={role} className="text-center px-3 py-2.5">
                            <Switch
                              checked={checked}
                              disabled={saving === key}
                              onCheckedChange={(v) => { setSaving(key); toggle.mutate({ role, route: route.path, value: v }); }}
