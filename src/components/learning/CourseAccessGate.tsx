...
export function CourseAccessGate({ course, children }: Props) {
  const { loading, canAccess, isAuthed } = useCourseAccess(course?.id);
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-10 text-center space-y-4 max-w-xl mx-auto">
      ...
      <div className="space-y-2">
        <h2 className="font-heading text-xl font-bold">Enroll to unlock</h2>
        <p className="text-sm text-muted-foreground">
          Lessons, quizzes and assignments are available to paid students only. Complete your
          enrollment to get full access to this course.
        </p>
      </div>
      <div className="flex flex-wrap items-center justify-center gap-2">
        <Button asChild>
          <Link to={courseHref(course)}>Enroll now</Link>
        </Button>
        {!isAuthed && (
